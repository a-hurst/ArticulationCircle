...
# 		
	...
	def trial(self):
		...
		while self.evi.before('circle_off', True):
			...
			if 0.9 < (self.start_time - self.trial_time) <= 1.050:
				print("init:", self.init_time - self.start_time)
				if lines_on:
					print("line:", self.line_time - self.start_time)
				if circle_on:
					print("circle:", self.circle_time - self.start_time)
				print("total:", self.elapsed)
