...
circle_width = 650
circle_radius = (circle_width + 16) // 2
response_bounds = [Params.screen_c, circle_radius - 32, circle_radius]
default_stroke = 5
...
class articulation_circle(klibs.Experiment):
	...
	def setup(self):
		...
		self.response_circle = kld.Annulus(circle_width, 32, fill=GREY)
	...
	def setup_response_collector(self):
		...
		self.rc.color_listener.add_boundary("color ring", response_bounds, ANULUS_BOUNDARY)
		...
		while self.evi.before('response_circle_on', True):
			...
			if self.evi.after('circle_on', True) and self.evi.before('circle_off', True):
				...
				if self.circle_type != "none":
					...
					#	self.blit(self.articulation_line_a, 5, util.point_pos(Params.screen_c, (circle_width / 2), 0))
					#	self.blit(self.articulation_line_b, 5, util.point_pos(Params.screen_c, (circle_width / 2), 45))
					#	self.blit(self.articulation_line_c, 5, util.point_pos(Params.screen_c, (circle_width / 2), 90))
					#	self.blit(self.articulation_line_d, 5, util.point_pos(Params.screen_c, (circle_width / 2), 135))
					#	self.blit(self.articulation_line_e, 5, util.point_pos(Params.screen_c, (circle_width / 2), 180))
					#	self.blit(self.articulation_line_f, 5, util.point_pos(Params.screen_c, (circle_width / 2), 225))
					#	self.blit(self.articulation_line_g, 5, util.point_pos(Params.screen_c, (circle_width / 2), 270))
					#	self.blit(self.articulation_line_h, 5, util.point_pos(Params.screen_c, (circle_width / 2), 315))
					#	#for i, deg in enumerate(articulation_angles):
					#	#	self.blit(self.articulations[i], 5, util.point_pos(Params.screen_c, (circle_width / 2), deg))
