...
# 		
	...
	def display_refresh(self, circle=False, lines=False, target=False):
		...
		if target:
			...
			print("Loop time after init:", after_init - start)
		...
		if lines:
			for l in self.lines:
				self.blit(l[0], 5, l[1])
				if target:
					after_lines = time.time()
					print("Loop time after lines:", after_lines - start)
# 			for deg in [0, 180]:
...
# 				self.blit(self.line_135, 5, line_pos)
		...
		if circle:
			...
			if target:
				...
				print("Loop time after circle:", after_circle - start)
