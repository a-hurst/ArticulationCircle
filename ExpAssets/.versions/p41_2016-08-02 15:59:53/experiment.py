...
class articulation_circle(klibs.Experiment):
	...
	def trial_prep(self):
		...
		self.target_angle = random.choice(range(0, 359, 1))
		self.rc.color_listener.rotation(self.target_angle)
	...
	def trial(self):
		...
		while self.evi.before('response_circle_on', True):
			...
			if self.evi.after('target_on', True) and self.evi.before('target_off', True):
				self.blit(self.target, 5, util.point_pos(Params.screen_c, (circle_width / 2), self.target_angle))
