...
class articulation_circle(klibs.Experiment):
	...
	def setup(self):
		...
		#	self.articulation = kld.Line(25, DARKGREY, default_stroke, i) # For whatever reason this doesn't work, maybe bug.
			...
			if self.evi.after('circle_on', True) and self.evi.before('circle_off', True):
				...
				if self.circle_type != "none":
					...
					if self.circle_type == "articulated":
						for i in articulations:
							self.articulation = kld.Line(25, DARKGREY, default_stroke, i)
							self.blit(self.articulation, 5, util.point_pos(Params.screen_c, (circle_width / 2), i))
